    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        <div class="rating-type border-1px">
          <span class="block positive" :class="{'active':selectType === 2}" @click="fn(2)">
            全部<span class="count">{{ratings.length}}</span>
          </span>
          <span class="block positive" :class="{'active':selectType === 0}" @click="fn(0)">
            满意<span class="count">{{positiveSize}}</span>
          </span>
          <span class="block negative" :class="{'active':selectType === 1}" @click="fn(1)">
            不满意<span class="count">{{ratings.length - positiveSize}}</span>
          </span>
        </div>
        <div class="switch" :class="onlyShowText ? 'on' : ''" @click="toggleShowText">
...
export default {
  data(){
    return{
      onlyShowText:false,//只看有内容的评价（是否只显示有文本的）
      selectType:2//评论的三个分类，0：满意，1：不满意，2：全部
    }
  },
computed:{
